/*
 * Wearable safety stream: Real-time bulletins from wearable / IoT streams
 *
 * Custom Feature Suggestion (apply pass 6 — audit batch_01)
 * Suggestion: Real-time bulletins from wearable / IoT streams
 *
...
router.post('/run', async (req, res) => {
  try {
    if (!process.env.OPENROUTER_API_KEY) {
      return res.status(503).json({ error: 'AI unavailable', missing: 'OPENROUTER_API_KEY', feature: 'wearable_safety_stream' });
    }
    const payload = req.body || {};
    const userPrompt = `Context (JSON):\n${JSON.stringify(payload).slice(0, 4000)}\n\nTask: ${'Wearable safety stream'}\n\nReturn JSON {"summary": string, "actions": [string], "risks": [string], "confidence": number_0_to_1}.`;
    const systemPrompt = `You are a $'wearable-data safety stream analyzer'. Respond with strict JSON.`;
    const aiResp = await callOpenRouter(userPrompt, systemPrompt);
    const content = aiResp?.choices?.[0]?.message?.content ?? aiResp?.raw ?? '';
    const parsed = parseAIJson(content);
    return res.json({ feature: 'wearable_safety_stream', model: aiResp?.model, parsed, raw: content });
  } catch (err) {
...
router.get('/health', (req, res) => res.json({ feature: 'wearable_safety_stream', ok: true, ai_configured: !!process.env.OPENROUTER_API_KEY }));
