...
export default function RealTimeStreamFromWearablesIotTriggeringInsPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/cf-real-time-stream-from-wearables-iot-triggering-ins/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Real-time stream from wearables/IoT triggering instant safety bulletins</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-real-time-stream-from-wearables-iot-triggering-ins/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
