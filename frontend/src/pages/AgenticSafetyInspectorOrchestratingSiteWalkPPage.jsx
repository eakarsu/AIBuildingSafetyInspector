...
export default function AgenticSafetyInspectorOrchestratingSiteWalkPPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/cf-agentic-safety-inspector-orchestrating-site-walk-p/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Agentic safety inspector orchestrating site walk, photo capture, code lookup, and corrective dispatch</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-agentic-safety-inspector-orchestrating-site-walk-p/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
